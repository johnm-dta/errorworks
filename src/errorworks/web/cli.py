...
def _version_callback(value: bool) -> None:
    ...
    if value:
        ...
        try:
            typer.echo(f"chaosweb (errorworks {version('errorworks')})")
...
def serve(
    ...
    if config.server.workers > 1:
        ...
        fd, config_path = tempfile.mkstemp(prefix="errorworks-web-", suffix=".json")
        os.close(fd)
        os.chmod(config_path, 0o600)
        Path(config_path).write_text(config.model_dump_json())
        os.environ["_ERRORWORKS_WEB_CONFIG_FILE"] = config_path
        try:
            uvicorn.run(
            ...
            os.environ.pop("_ERRORWORKS_WEB_CONFIG_FILE", None)
